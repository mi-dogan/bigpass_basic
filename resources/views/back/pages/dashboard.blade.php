...
<div class="app-main flex-column flex-row-fluid" id="kt_app_main">
    ...
    <script>
        function modal(id) {
            ...
            var Ajaxurl = '{{ route('raporlar.show',':id') }}';
            Ajaxurl = Ajaxurl.replace(':id', id);
            var url = '{{ route('raporlar.update',':id') }}';
          ...
          Ajaxurl = Ajaxurl.replace(':id',id);
            ...
            success: function (response) {
                if(response.activity.shiftDay)
                {
                    ...
                    var realMorningExit = new Date(todayDate + 'T' + response.activity.shiftDay.morning_exit);
                    var exitDifference = Math.abs(morningExit - realMorningExit) / 1000;
                    morningExitColor = (exitDifference >= optionInSeconds) ? 'danger' : 'success';
                    var exithours = Math.floor(exitDifference / 3600);
                    var exitminutes = Math.floor((exitDifference % 3600) / 60);
                    }
                }
                ...
                <div class="fs-6 d-flex justify-content-between mb-4">
                    ...
                    <div class="d-flex fw-bold text-${noonExitColor ?? 'success'}">${response.activity.noon_exit ?? (response.activity.status == 1 ? 'Gelmedi' : (response.activity.status == 2 ? 'İzinli' : '-'))}</div>
                </div>
                <div class="separator separator-dashed"></div>
                <div class="fs-4 fw-semibold text-gray-400 mb-7"></div>
                <div class="fs-6 d-flex justify-content-between mb-4">
                    <div class="fw-semibold">Öğle arası giriş</div>
                    <div class="d-flex fw-bold text-${noonEntranceColor ?? 'success'}">${response.activity.noon_entrance ?? (response.activity.status == 1 ? 'Gelmedi' : (response.activity.status == 2 ? 'İzinli' : '-'))}</div>
