        ...
        <div id="kt_app_toolbar_container" class="app-container container-fluid d-flex flex-stack">
            <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                    ...
                    <li class="breadcrumb-item text-muted">Resmi Tatiller</li>
                </ul>
            </div>
            <div class="d-flex align-items-center gap-2 gap-lg-3">
                <a href="#" class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#kt_modal_create_holiday">Tatil ekle</a>
            </div>
        </div>
    </div>
								...
								<tr style="font-size: 13px;">
								    <td class="text-start px-md-12 px-6 py-6">
                                     ...
                                     <td class="text-center px-md-12 px-6 py-6">
                                        {{ str()->limit($holiday->day->translatedFormat('d F Y')) }}
                                     </td>
								    <td class="text-end px-md-12 px-6 py-4">
								        <div class="text-end">
								           <div class="d-flex align-items-center justify-content-end">
								            <a onclick="modal({{$holiday->id}})" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1 bg-gray-200i">
								            ...
								            <a onclick="deleteHoliday('{{route('tatiller.delete',$holiday->id)}}')" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm bg-gray-200i">
...
<div class="modal fade draggable" id="kt_modal_create_holiday" tabindex="-1" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
    <div class="modal-dialog modal-dialog-centered mw-500px">
        <div class="modal-content">
            <form method="post" id="kt_modal_create_holiday" class="form w-100 needs-validation" action="{{route('tatiller.store')}}">
                @csrf
                <div class="modal-header ui-draggable-handle py-5" id="kt_modal_add_customer_header">
                    <h3 class="fw-bolder">Tatil Ekle</h3>
                ...
                <div class="modal-body py-5 px-lg-17">
                    <div class="scroll-y me-n7 pe-7" id="kt_modal_add_customer_scroll" data-kt-scroll="true" data-kt-scroll-activate="{default: false, lg: true}" data-kt-scroll-max-height="auto" data-kt-scroll-dependencies="#kt_modal_add_customer_header" data-kt-scroll-wrappers="#kt_modal_add_customer_scroll" data-kt-scroll-offset="300px">
                         <div class="fv-row mb-5">
                             <label class="form-label fs-7">Başlık</label>
                         ...
                         <div class="fv-row mb-5">
                             ...
                             <input class="form-control form-control-lg bg-transparent @error('date') border-danger @enderror" type="date" name="date" value="{{old('date')}}" required>
                             @error('date')
                             <div class="invalid-feedback d-block text-danger f-11">{{$message}}</div>
                             @enderror
                         </div>
                ...
                <div class="modal-footer flex-center py-5">
                    ...
                    <button type="submit" class="btn btn-primary min-w-200px"><i class="las la-check-double fs-2"></i>Tatil Ekle</button>
                </div>
            </form>
        </div>
    </div>
</div>
<div class="modal fade draggable" id="kt_modal_edit_holiday" tabindex="-1" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
    <div class="modal-dialog modal-dialog-centered mw-500px py-2">
        <div class="modal-content">
            <form method="post" id="kt_modal_edit_holiday_form" class="form w-100">
                @method('PUT')
                @csrf
                ...
                <div class="modal-body py-5 px-lg-17">
                    <div class="scroll-y me-n7 pe-7" id="kt_modal_add_customer_scroll" data-kt-scroll="true" data-kt-scroll-activate="{default: false, lg: true}" data-kt-scroll-max-height="auto" data-kt-scroll-dependencies="#kt_modal_add_customer_header" data-kt-scroll-wrappers="#kt_modal_add_customer_scroll" data-kt-scroll-offset="300px">
                        <div class="fv-row mb-5">
                            <label class="form-label fs-7">Başlık</label>
                            <input class="form-control form-control-lg bg-transparent @error('update_name') border-danger @enderror" type="text" name="update_name" id="update_name" value="{{old('update_name')}}" required>
                            @error('update_name')
                            <div class="invalid-feedback d-block text-danger f-11">{{$message}}</div>
                            @enderror
                        </div>
                        <div class="fv-row mb-5">
                            <label class="form-label fs-7">Tarih</label>
                            <input class="form-control form-control-lg bg-transparent @error('update_date') border-danger @enderror" type="date" name="update_date" id="update_date" value="2018-07-22" required>
                            @error('update_date')
                            <div class="invalid-feedback d-block text-danger f-11">{{$message}}</div>
                            @enderror
                        </div>
...
<script>
    function modal(id)
    {
        ...
        var Ajaxurl = '{{ route('tatiller.show', ':id') }}';
        Ajaxurl = Ajaxurl.replace(':id', id);
        var url = '{{ route('tatiller.update', ':id') }}';
        url = url.replace(':id', id);
         $.ajax({
            ...
            success: function (response) {
                var dateString = response.holiday.day;
                var date = new Date(dateString);
                var formattedDate = date.toISOString().slice(0, 10);
                $('#update_name').val(response.holiday.name);
                $('#update_date').val(formattedDate);
                $('#kt_modal_edit_holiday_form').attr('action',url);
                $('#kt_modal_edit_holiday').modal('show');
            }
         });
    }
    ...
    function deleteHoliday(url){
      Swal.fire({
          title: 'Uyarı!',
          text:'Tatili Silmek istediğinizden emin misiniz?',
          icon: 'warning',
...
<script>
   $(document).ready(() => {
     setTimeout(() => {
         $('#kt_modal_create_holiday').modal('show');
     }, 500);
   });
</script>
@endif
