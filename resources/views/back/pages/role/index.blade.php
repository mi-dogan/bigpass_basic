    ...
    <div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6 col-12">
        <div id="kt_app_toolbar_container" class="app-container container-fluid d-flex flex-stack">
            ...
            <div class="d-flex align-items-center gap-2 gap-lg-3">
                <a href="#" class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#kt_modal_create_role">Rol ekle</a>
            </div>
        </div>
    </div>
                    ...
                    <table class="table table-striped border rounded table-row-dashed fs-6  px-0 mx-0 overflow-x-scroll" id="kt_datatable_example">
                        <thead class="py-12 px-2">
                            <tr class="text-gray-700 fw-bold text-uppercase bg-light w-100 px-0 mx-0">
                                  ...
                                  <th class="text-center px-md-12 px-6 w-25">Kullanıcı</th>
								    ...
								    <td class="text-end px-md-12 px-6 py-4">
								        <div class="text-end">
								           <div class="d-flex align-items-center justify-content-end">
								            <a onclick="modal({{$role->id}})" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1 bg-gray-200i">
								            ...
								            <a onclick="deleteRole('{{route('roller.delete',$role->id)}}')" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm bg-gray-200i">
...
<div class="modal fade draggable" id="kt_modal_create_role" tabindex="-1" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
    <div class="modal-dialog modal-dialog-centered mw-500px">
        <div class="modal-content">
            <form method="post" id="kt_modal_create_role" class="form w-100 needs-validation" action="{{route('roller.store')}}">
                @csrf
                <div class="modal-header ui-draggable-handle py-5" id="kt_modal_add_customer_header">
                    <h3 class="fw-bolder">Rol Ekle</h3>
                ...
                <div class="modal-body py-5 px-12">
                    <div class="scroll-y " id="kt_modal_add_customer_scroll" data-kt-scroll="true" data-kt-scroll-activate="{default: false, lg: true}" data-kt-scroll-max-height="auto" data-kt-scroll-dependencies="#kt_modal_add_customer_header" data-kt-scroll-wrappers="#kt_modal_add_customer_scroll" data-kt-scroll-offset="300px">
                         <div class="fv-row mb-5">
                             <label class="form-label fs-7">Rol Adı</label>
                ...
                <div class="modal-footer flex-center py-2">
                    ...
                    <button type="submit" class="btn btn-primary min-w-200px"><i class="las la-check-double fs-2"></i>Rol Ekle </button>
                </div>
            </form>
        </div>
    </div>
</div>
<div class="modal fade draggable" id="kt_modal_edit_role" tabindex="-1" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
    <div class="modal-dialog modal-dialog-centered mw-500px py-2">
        <div class="modal-content">
            <form method="post" id="kt_modal_edit_role_form" class="form w-100">
                @method('PUT')
                @csrf
                <div class="modal-header py-5">
                    <h3 class="fw-bolder">Rol Düzenle</h3>
                    <div class="btn btn-icon btn-sm btn-active-light-primary ms-2" data-bs-dismiss="modal" aria-label="Close">
                        <span class="svg-icon svg-icon-2x">
                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">
                                <g transform="translate(12.000000, 12.000000) rotate(-45.000000) translate(-12.000000, -12.000000) translate(4.000000, 4.000000)" fill="#000000">
...
<script>
    function modal(id)
    {
        ...
        var Ajaxurl = '{{ route('roller.show', ':id') }}';
        Ajaxurl = Ajaxurl.replace(':id', id);
        var url = '{{ route('roller.update', ':id') }}';
        url = url.replace(':id', id);
         $.ajax({
            ...
            success: function (response) {
                $('#update_name').val(response.role.name);
                $('#kt_modal_edit_role_form').attr('action',url);
    ...
    function deleteRole(url){
      Swal.fire({
          title: 'Uyarı!',
          text:'Rolü Silmek istediğinizden emin misiniz?',
          icon: 'warning',
