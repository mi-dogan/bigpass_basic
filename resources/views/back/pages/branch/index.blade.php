    ...
    <div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6 col-12">
        <div id="kt_app_toolbar_container" class="app-container container-fluid d-flex flex-stack">
            ...
            <div class="d-flex align-items-center gap-2 gap-lg-3">
                <a  class="btn btn-sm btn-primary" id="addButton" data-bs-toggle="modal">Şube ekle</a>
            </div>
        </div>
    </div>
                    ...
                    <table class="table table-striped border rounded table-row-dashed fs-6  px-0 mx-0 overflow-x-scroll" id="kt_datatable_example">
                        <thead class="py-12 px-2">
                            <tr class="text-gray-900 fw-bold text-capitalized bg-light w-100 px-0 mx-0"style="font-size: 15px;">
                                  <th class="text-start px-md-12 px-6 w-25">Şube</th>
                                  <th class="text-center px-md-12 px-6 w-25">Departman</th>
							 ...
							 @foreach ($branchs as $branch)
								<tr style="font-size: 13px;">
								    <td class="text-start px-md-12 px-6 py-6">
								       {{ str()->limit($branch->name,50) }}
								    </td>
                                    <td class="text-center px-md-12 px-6 py-6">
                                      <span class="badge badge-light-primary">{{$branch->departments_count}}</span>
                                    </td>
								    <td class="text-end px-md-12 px-6 py-4">
								        <div class="text-end">
								           <div class="d-flex align-items-center justify-content-end">
								            <a onclick="modal({{$branch->id}})" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1 bg-gray-200i">
								            ...
								            <a onclick="deleteDepartment('{{route('subeler.delete',$branch->id)}}')" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm bg-gray-200i">
...
<div class="modal fade draggable" id="kt_modal_create_branch" tabindex="-1" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
    <div class="modal-dialog modal-dialog-centered mw-500px">
        <div class="modal-content">
            <form method="post" id="kt_modal_create_branch" class="form w-100 needs-validation" action="{{route('subeler.store')}}">
                @csrf
                <div class="modal-header ui-draggable-handle py-5" id="kt_modal_add_customer_header">
                    <h3 class="fw-bolder">Şube Ekle</h3>
                ...
                <div class="modal-body py-5 px-12">
                    <div class="scroll-y " id="kt_modal_add_customer_scroll" data-kt-scroll="true" data-kt-scroll-activate="{default: false, lg: true}" data-kt-scroll-max-height="auto" data-kt-scroll-dependencies="#kt_modal_add_customer_header" data-kt-scroll-wrappers="#kt_modal_add_customer_scroll" data-kt-scroll-offset="300px">
                         <div class="fv-row mb-5">
                             <label class="form-label fs-7">Şube Adı</label>
                ...
                <div class="modal-footer flex-center py-2">
                    ...
                    <button type="submit" class="btn btn-primary min-w-200px"><i class="las la-check-double fs-2"></i>Şube Ekle </button>
                </div>
            </form>
        </div>
    </div>
</div>
<div class="modal fade draggable" id="kt_modal_edit_branch" tabindex="-1" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
    <div class="modal-dialog modal-dialog-centered mw-500px py-2">
        <div class="modal-content">
            <form method="post" id="kt_modal_edit_branch_form" class="form w-100">
                @method('PUT')
                @csrf
...
<script>
    function modal(id)
    {
        ...
        var Ajaxurl = '{{ route('subeler.show', ':id') }}';
        Ajaxurl = Ajaxurl.replace(':id', id);
        var url = '{{ route('subeler.update', ':id') }}';
        url = url.replace(':id', id);
         $.ajax({
            ...
            success: function (response) {
                ...
                $('#kt_modal_edit_branch_form').attr('action',url);
                $('#kt_modal_edit_branch').modal('show');
            }
         });
    }
    ...
    function deleteDepartment(url){
      Swal.fire({
          title: 'Uyarı!',
          text:'Şubeyi Silmek istediğinizden emin misiniz?',
          icon: 'warning',
...
<script>
   $(document).ready(() => {
    consolee.log("getted readey")
     setTimeout(() => {
         $('#kt_modal_create_branch').modal('show');
     }, 500);
   });
</script>
@endif
      ...
      $.ajax({
        ...
        success: function (response) {
          if (response.limitReached) {
            ...
            $('#kt_modal_create_branch').modal('show'); // Show the modal if limit is not reached
          }
        },
        error: function () {
          alert('Error checking branch limit.');
        }
      });
    });
  });
