    ...
    <div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6 col-12">
        <div id="kt_app_toolbar_container" class="app-container container-fluid d-flex flex-stack">
            ...
            <div class="d-flex align-items-center gap-2 gap-lg-3">
                <a href="#" class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#kt_modal_create_user">Kullanıcı ekle</a>
            </div>
        </div>
    </div>
                    ...
                    <table class="table table-striped border rounded table-row-dashed fs-6  px-0 mx-0 overflow-x-scroll" id="kt_datatable_example">
                        <thead class="py-12 px-2">
                            <tr class="text-gray-700 fw-bold text-uppercase bg-light w-100 px-0 mx-0">
                                  ...
                                  <th class="text-start px-md-12 px-6 w-25">Rol</th>
								    ...
								    <td class="text-end px-md-12 px-6 py-4">
								        <div class="text-end">
								           <div class="d-flex align-items-center justify-content-end">
								            <a onclick="modal({{$user->id}})" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1 bg-gray-200i">
								            ...
								            <a onclick="deleteUser('{{route('kullanicilar.delete',$user->id)}}')" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm bg-gray-200i">
...
<div class="modal fade draggable" id="kt_modal_create_user" tabindex="-1" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
    <div class="modal-dialog modal-dialog-centered mw-650px">
        <div class="modal-content">
            <form method="post" id="kt_modal_create_user" class="form w-100 needs-validation" action="{{route('kullanicilar.store')}}">
                @csrf
                <div class="modal-header ui-draggable-handle py-5" id="kt_modal_add_customer_header">
                    <h3 class="fw-bolder">Kullanıcı Ekle</h3>
                ...
                <div class="modal-body py-5 px-lg-17">
                    <div class="scroll-y me-n7 pe-7" id="kt_modal_add_customer_scroll" data-kt-scroll="true" data-kt-scroll-activate="{default: false, lg: true}" data-kt-scroll-max-height="auto" data-kt-scroll-dependencies="#kt_modal_add_customer_header" data-kt-scroll-wrappers="#kt_modal_add_customer_scroll" data-kt-scroll-offset="300px">
                         <div class="fv-row mb-5">
                             <label class="form-label fw-bolder text-dark fs-6">Ad Soyad</label>
                         ...
                         <div class="fv-row mb-5">
                             <div class="d-flex flex-stack">
                                 <span class="form-label fw-bolder text-dark fs-6" data-kt-element="targets-previous">E-posta Adresi</span>
                                 <div id="Sonuc" class="text-danger mb-2" data-kt-element="targets-next"></div>
                             </div>
                             <input class="form-control form-control-lg bg-transparent @error('email') border-danger @enderror" type="email" id="email" name="email" value="{{old('email')}}" required="">
                             @error('email')
                             <div class="invalid-feedback d-block text-danger f-11">{{$message}}</div>
                             @enderror
                         </div>
                         <div class="mb-5 fv-row" data-kt-password-meter="true">
                             <div class="mb-1">
                                 ...
                                 <div class="position-relative mb-3">
                                     <input class="form-control form-control-lg bg-transparent @error('password') border-danger @enderror" type="password" name="password" value="{{old('password')}}" required="">
                         ...
                         <div class="mb-5 fv-row" data-kt-password-meter="true">
                             <div class="mb-1">
                                 <label class="form-label fw-bolder text-dark fs-6">Şifre Tekrar</label>
                                 <div class="position-relative mb-3">
                                     <input class="form-control form-control-lg bg-transparent @error('password_confirmation') border-danger @enderror" type="password" name="password_confirmation" value="{{old('password_confirmation')}}" required="">
                                     <span class="btn btn-sm btn-icon position-absolute translate-middle top-50 end-0 me-n2" data-kt-password-meter-control="visibility">
                                         <i class="bi bi-eye-slash fs-2"></i>
                                         <i class="bi bi-eye fs-2 d-none"></i>
                                     </span>
                         ...
                         <div class="fv-row mb-5">
                             <label class="form-label fw-bolder text-dark fs-6">Departman</label>
                             <select name="department_id[]" class="form-select form-control-lg @error('department_id') border-danger @enderror" data-dropdown-parent="#kt_modal_create_user" data-allow-clear="true" data-control="select2" data-placeholder="Departman Seçiniz" data-close-on-select="false" multiple="multiple">
                                 @foreach ($departments as $department)
                                 <option value="{{$department->id}}">{{$department->branch->name . ' - ' .$department->name}}</option>
                                 @endforeach
                             </select>
                             @error('department_id')
                             <div class="invalid-feedback d-block text-danger f-11">{{$message}}</div>
                ...
                <div class="modal-footer flex-center py-5">
                    ...
                    <button type="submit" class="btn btn-primary min-w-200px"><i class="las la-check-double fs-2"></i>Kullanıcı Ekle </button>
                </div>
            </form>
        </div>
    </div>
</div>
<div class="modal fade draggable" id="kt_modal_edit_user" tabindex="-1" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
    <div class="modal-dialog modal-dialog-centered mw-650px py-2">
        <div class="modal-content">
            <form method="post" id="kt_modal_edit_user_form" class="form w-100">
                @method('PUT')
                @csrf
                <div class="modal-header py-5">
                    <h3 class="fw-bolder">Kullanıcı Düzenle</h3>
                    <div class="btn btn-icon btn-sm btn-active-light-primary ms-2" data-bs-dismiss="modal" aria-label="Close">
                        <span class="svg-icon svg-icon-2x">
                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">
                                <g transform="translate(12.000000, 12.000000) rotate(-45.000000) translate(-12.000000, -12.000000) translate(4.000000, 4.000000)" fill="#000000">
                ...
                <div class="modal-body py-5 px-lg-17">
                    <div class="scroll-y me-n7 pe-7" id="kt_modal_add_customer_scroll" data-kt-scroll="true" data-kt-scroll-activate="{default: false, lg: true}" data-kt-scroll-max-height="auto" data-kt-scroll-dependencies="#kt_modal_add_customer_header" data-kt-scroll-wrappers="#kt_modal_add_customer_scroll" data-kt-scroll-offset="300px">
                        ...
                        <div class="fv-row mb-5">
                            ...
                            <input class="form-control form-control-lg bg-transparent @error('update_email') border-danger @enderror" type="email" id="update_email" name="update_email" id="update_email" value="{{old('update_email')}}" required>
                            @error('update_email')
                            <div class="invalid-feedback d-block text-danger f-11">{{$message}}</div>
                            @enderror
                        </div>
                         <div class="mb-5 fv-row" data-kt-password-meter="true">
                             <div class="mb-1">
                                 <label class="form-label fw-bolder text-dark fs-6">Şifre</label>
                                 <div class="position-relative mb-3">
                                     <input class="form-control form-control-lg bg-transparent @error('update_password') border-danger @enderror" type="password" name="update_password" value="{{old('update_password')}}">
                                     <span class="btn btn-sm btn-icon position-absolute translate-middle top-50 end-0 me-n2" data-kt-password-meter-control="visibility">
                                         <i class="bi bi-eye-slash fs-2"></i>
                                         <i class="bi bi-eye fs-2 d-none"></i>
                                     </span>
                         ...
                         <div class="mb-5 fv-row" data-kt-password-meter="true">
                             <div class="mb-1">
                                 <label class="form-label fw-bolder text-dark fs-6">Şifre Tekrar</label>
                                 <div class="position-relative mb-3">
                                     <input class="form-control form-control-lg bg-transparent @error('update_password_confirmation') border-danger @enderror" type="password" name="update_password_confirmation" value="{{old('password_confirmation')}}">
                                     <span class="btn btn-sm btn-icon position-absolute translate-middle top-50 end-0 me-n2" data-kt-password-meter-control="visibility">
                                         <i class="bi bi-eye-slash fs-2"></i>
                                         <i class="bi bi-eye fs-2 d-none"></i>
                                     </span>
                        ...
                        <div class="fv-row mb-5">
                            <label class="form-label fw-bolder text-dark fs-6">Departman</label>
                            <select name="update_department_id[]" class="form-select form-control-lg @error('update_department_id') border-danger @enderror" id="update_branch_id" data-dropdown-parent="#kt_modal_edit_user" data-allow-clear="true" data-control="select2" data-placeholder="Departman Seçiniz" data-close-on-select="false" multiple="multiple">
                                @foreach ($departments as $department)
                                <option value="{{$department->id}}">{{$department->branch->name . ' - ' .$department->name}}</option>
                                @endforeach
                            </select>
                            @error('update_department_id')
                            <div class="invalid-feedback d-block text-danger f-11">{{$message}}</div>
                            @enderror
                        </div>
                        <div class="fv-row mb-5">
                            <label class="form-check form-check-custom form-control-lg bg-transparent me-10 fw-bold">
                                <input class="form-check-input h-20px w-20px" type="checkbox" id="update_admin" value="1" @checked(old('update_admin')) name="update_admin">
                                <span class="form-check-label fw-bold fs-5">Kullanıcıyı Yönetici Yap
                                </span>
                            </label>
                        </div>
...
<script>
    function modal(id)
    {
        ...
        var Ajaxurl = '{{ route('kullanicilar.show', ':id') }}';
        ...
        url = url.replace(':id', id);
         $.ajax({
            ...
            success: function (response) {
                ...
                var branchSelect = $('#update_branch_id').select2();
                branchSelect.find('option').prop('selected', false);
                branchSelect.find('option').each(function() {
                    var optionValue = $(this).val();
                    const userDepartments = response.user.departments.map(department => department.department_id);
                    if (userDepartments.includes(parseInt(optionValue))) {
                        $(this).prop('selected', true);
                    }
                    branchSelect.trigger('change');
                });
                $('#update_admin').prop('checked',response.user.roles[0].name == 'admin' ? true : false );
                $('#kt_modal_edit_user_form').attr('action',url);
