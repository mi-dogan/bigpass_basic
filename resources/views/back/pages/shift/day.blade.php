        ...
        <div id="kt_app_toolbar_container" class="app-container container-fluid d-flex flex-stack">
            <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                    ...
                    <li class="breadcrumb-item text-muted">
                        <a href="{{route('vardiyalar.index')}}" class="text-muted text-hover-primary">Vardiyalar</a>
                    </li>
                     <li class="breadcrumb-item">
                         <span class="bullet bg-gray-400 w-5px h-2px"></span>
                     </li>
                    <li class="breadcrumb-item text-muted">Vardiya Günleri</li>
                </ul>
            </div>
            <div class="d-flex align-items-center gap-2 gap-lg-3">
                <a href="#" class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#kt_modal_create_day">Gün ekle</a>
            </div>
        </div>
    </div>
							 ...
							 @foreach ($days as $day)
                                 <tr>
                                     <td class="text-start px-md-12 px-6 py-6">
                                         {{ $day->type == 1 ? 'Pazartesi' : ($day->type == 2 ?  'Salı' : ($day->type == 3 ? 'Çarşamba' : ($day->type == 4 ? 'Perşembe' : ($day->type == 5 ? 'Cuma' : ($day->type == 6 ? 'Cumartesi' : 'Pazar'))))) }}
                                     </td>
                                     <td class="text-start px-md-12 px-6 py-6">
                                      {{substr($day->morning_entrance, 0, 5)}}
                                     </td>
                                     <td class="text-start px-md-12 px-6 py-6">
                                       {{substr($day->morning_exit,0,5)}}
                                     </td>
                                      <td class="text-start px-md-12 px-6 py-6">
                                       {{substr($day->option,0,5)}}
                                     </td>
                                     <td class="text-end px-md-12 px-6 py-4">
                                         <div class="text-end">
                                             <div class="d-flex align-items-center justify-content-end">
                                                 <a onclick="modal({{$day->id}})" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1 bg-gray-200i">
                                                 ...
                                                 <a onclick="deleteShift('{{ route('gunler.delete', ['id' => $shift->id, 'gunler' => $day->id]) }}')" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm bg-gray-200i">
...
<div class="modal fade draggable" id="kt_modal_create_day" tabindex="-1" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
    <div class="modal-dialog modal-dialog-centered mw-650px">
        <div class="modal-content">
            <form method="post" id="kt_modal_create_day" class="form w-100 needs-validation" action="{{route('gunler.store',$shift->id)}}">
                @csrf
                <div class="modal-header ui-draggable-handle py-5" id="kt_modal_add_customer_header">
                    <h3 class="fw-bolder">Gün Ekle</h3>
                ...
                <div class="modal-body py-5 px-lg-17">
                    <div class="scroll-y me-n7 pe-7" id="kt_modal_add_customer_scroll" data-kt-scroll="true" data-kt-scroll-activate="{default: false, lg: true}" data-kt-scroll-max-height="auto" data-kt-scroll-dependencies="#kt_modal_add_customer_header" data-kt-scroll-wrappers="#kt_modal_add_customer_scroll" data-kt-scroll-offset="300px">
                         <div class="fv-row mb-5">
                             <label class="form-label fs-7">Gün</label>
                             <select name="type" class="form-select form-control-lg @error('type') border-danger @enderror" data-dropdown-parent="#kt_modal_create_day" data-control="select2">
                                @if (!in_array(1, $days->pluck('type')->toArray()))
                                <option {{ old('type') == 1 ? 'selected' : '' }} value="1">Pazartesi</option>
                                @endif
                                @if (!in_array(2, $days->pluck('type')->toArray()))
                                <option {{ old('type') == 2 ? 'selected' : '' }} value="2">Salı</option>
                                @endif
                                @if (!in_array(3, $days->pluck('type')->toArray()))
                                <option {{ old('type') == 3 ? 'selected' : '' }} value="3">Çarşamba</option>
                                @endif
                                @if (!in_array(4, $days->pluck('type')->toArray()))
                                <option {{ old('type') == 4 ? 'selected' : '' }} value="4">Perşembe</option>
                                @endif
                                @if (!in_array(5, $days->pluck('type')->toArray()))
                                <option {{ old('type') == 5 ? 'selected' : '' }} value="5">Cuma</option>
                                @endif
                                @if (!in_array(6, $days->pluck('type')->toArray()))
                                <option {{ old('type') == 6 ? 'selected' : '' }} value="6">Cumartesi</option>
                                @endif
                                @if (!in_array(7, $days->pluck('type')->toArray()))
                                <option {{ old('type') == 7 ? 'selected' : '' }} value="7">Pazar</option>
                                @endif
                             </select>
                             @error('type')
...
<div class="modal fade draggable" id="kt_modal_edit_day" tabindex="-1" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
    <div class="modal-dialog modal-dialog-centered mw-650px py-2">
        <div class="modal-content">
            <form method="post" id="kt_modal_edit_day_form" class="form w-100">
                @method('PUT')
                @csrf
                <div class="modal-header py-5">
                    <h3 class="fw-bolder">Gün Düzenle</h3>
                    <div class="btn btn-icon btn-sm btn-active-light-primary ms-2" data-bs-dismiss="modal" aria-label="Close">
                        <span class="svg-icon svg-icon-2x">
                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">
                                <g transform="translate(12.000000, 12.000000) rotate(-45.000000) translate(-12.000000, -12.000000) translate(4.000000, 4.000000)" fill="#000000">
                ...
                <div class="modal-body py-5 px-lg-17">
                    <div class="scroll-y me-n7 pe-7" id="kt_modal_add_customer_scroll" data-kt-scroll="true" data-kt-scroll-activate="{default: false, lg: true}" data-kt-scroll-max-height="auto" data-kt-scroll-dependencies="#kt_modal_add_customer_header" data-kt-scroll-wrappers="#kt_modal_add_customer_scroll" data-kt-scroll-offset="300px">
                          <div class="fv-row mb-5">
                             <label class="form-label fs-7">Gün</label>
                             <select name="update_type" class="form-select form-control-lg @error('update_type') border-danger @enderror" id="type-select" data-dropdown-parent="#kt_modal_edit_day" data-control="select2">
                           ...
                           <div class="row row-cols-1 row-cols-sm-2 rol-cols-md-1 row-cols-lg-2">
                               <div class="col">
                                   <div class="fv-row mb-5">
                                       <label class="form-label fs-7">Sabah Giriş</label>
                           ...
                           <div class="row row-cols-1 row-cols-sm-2 rol-cols-md-1 row-cols-lg-2">
                               ...
                               <div class="col">
                                   <div class="fv-row mb-5">
                                       <label class="form-label fs-7">Öğlen Çıkış</label>
                                       <input class="form-control form-control-lg bg-transparent @error('update_noon_exit') border-danger @enderror" id="noon_exit" type="time" name="update_noon_exit" value="{{old('update_noon_exit')}}" required>
                           ...
                           <div class="fv-row mb-5">
                               <label class="form-label fs-7">Option</label>
                               <input class="form-control form-control-lg bg-transparent @error('update_option') border-danger @enderror" id="option" type="time" name="update_option" value="{{old('update_option')}}" required>
                               @error('update_option')
                               <div class="invalid-feedback d-block text-danger f-11">{{$message}}</div>
                               @enderror
                           </div>
...
<script>
    function modal(id)
    {
        ...
        var Ajaxurl = '{{ route('gunler.show',['id' => ':id','gunler'=> ':id2']) }}';
        Ajaxurl = Ajaxurl.replace(':id', {{$shift->id}}).replace(':id2',id);
        var url = '{{ route('gunler.update', ['id' => ':id','gunler'=> ':id2']) }}';
        url = url.replace(':id', {{$shift->id}}).replace(':id2',id);
         $.ajax({
            type: "get",
            url: Ajaxurl,
            data: {id: id},
            success: function (response) {
                 var status = response.day.status ? 1 : 0;
                 var typeSelect = $('#type-select').select2();
                 typeSelect.find('option').each(function() {
                 var optionValue = $(this).val();
                 if (optionValue == response.day.type) {
                 $(this).prop('selected', true);
                 typeSelect.trigger('change');
                 }
                 });
                 typeSelect.prop('disabled',true);
                 $('#morning_entrance').val(response.day.morning_entrance.substring(0, 5));
                 $('#morning_exit').val(response.day.morning_exit.substring(0, 5));
                 //$('#noon_entrance').val(response.day.noon_entrance.substring(0, 5));
                 //$('#noon_exit').val(response.day.noon_exit.substring(0, 5));
                 $('#option').val(response.day.option.substring(0, 5));
                $('#kt_modal_edit_day_form').attr('action',url);
                $('#kt_modal_edit_day').modal('show');
            }
         });
    }
    ...
    function deleteShift(url){
      Swal.fire({
          title: 'Uyarı!',
          text:'Günü Silmek istediğinizden emin misiniz?',
          icon: 'warning',
...
<script>
   $(document).ready(() => {
     setTimeout(() => {
         $('#kt_modal_create_day').modal('show');
     }, 500);
   });
</script>
@endif
...
<script>
    $(document).ready(() => {
        var url = '{{ route('gunler.update', ['id' => ':id','gunler'=> ':id2']) }}';
        url = url.replace(':id', {{$shift->id}}).replace(':id2',localStorage.getItem('id'));
       $('#kt_modal_edit_day_form').attr('action',url);
       setTimeout(() => {
         $('#kt_modal_edit_day').modal('show');
       }, 500);
